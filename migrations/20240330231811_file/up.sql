...
file_version has zero or more objects; every object has one or more file_versions. Any
set of changes is made as a new file_version.

(TODO: file_versions are currently linear: The latest file_version is implicitly the
most recent one, and it is implicitly based on the previous one in time. Adding a
"base_id" would enable a tree of file_versions; also adding a "merge_base_id" would
enable 2-way merges and file_versions to be a DAG.)
*/
